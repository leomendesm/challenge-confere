...
describe('Section reducer', () => {
    ...
    describe('fetching data', () => {
        it('should return isFetching true', () => {
            ...
            const action = deepFreeze({ type: FETCHING_SECTION_DATA })
            const after = {
                isFetching: true,
                data: null,
                hasError: false,
        ...
        it('should return success after fetching', () => {
            const before = deepFreeze(initialState)
            const action = deepFreeze({ type: FETCHING_SECTION_DATA_SUCCESS, payload: ['a'] })
            const after = {
                isFetching: false,
                data: ['a'],
                hasError: false,
        ...
        it('should return isFetching false and error', () => {
            const before = deepFreeze(initialState)
            const action = deepFreeze({ type: FETCHING_SECTION_DATA_FAIL, payload: '404' })
            const after = {
                isFetching: false,
                data: null,
                hasError: true,
